...
if (mysqli_num_rows($result) <= 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {  // iterate over result tuples
        ...
        // propriétés fausses
        if (!$row["mécanique"]) { echo '<li class="overline"> Mécanique </li>'; }
        if (!$row["split"]) { echo '<li class="overline"> Split </li>'; }
        if (!$row["deux_parties"]) { echo '<li class="overline"> Deux parties </li>'; }
        if (!$row["columnar"]) { echo '<li class="overline"> Columnar </li>'; }
        if (!$row["orthogonal"]) { echo '<li class="overline"> Orthogonal </li>'; }
        if (!$row["manuform"]) { echo '<li class="overline"> Manuform </li>'; }
        if (!$row["programmable"]) { echo '<li class="overline"> Programmable </li>'; }
        if (!$row["bluetooth"]) { echo '<li class="overline"> Bluetooth </li>'; }
        echo '</ul>';
